...
2. Will the native rust code cause issues?
3. Will the connectors cause issues?
4. Will the custom headers cause issues?
5. Will the complex webpack configuration on desktop cause issues?
6. Why does the component library have [[*/libs/components/tsconfig.app.json][two tsconfigs?]]
7. Will storybook cause issues?
8. Will manifest v2/3 cause issue?
...
***** =/apps/browser/=
The browser extension is a complex application that requires special considerations due to its nature as a browser extension and the need to build different variants for different browsers.
